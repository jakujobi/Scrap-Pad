...
for x in range(0, 7):  # Try values from 0 to 7 for x
    for y in range(0, 7):  # Try values from 0 to 7 for y
        for z in range(0, 7):  # Try values from 0 to 7 for z
            # Check if the values satisfy the equation
            if (x + 2 * y + 3 * z == 5) and (x+y+z < 7):
                # Add solution to the list
                solutions.append((x, y, z))
                Combinations.append((math.comb(6, z) * math.comb(6-z, y) * math.comb(6-z-y, x)))
